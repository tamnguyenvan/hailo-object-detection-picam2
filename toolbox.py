...
def init_input_source(input_path, batch_size, camera_resolution):
    ...
    Supports USB camera, RPi camera (Picamera2), video files, image dirs.
    """
    cap = None
    images = None
    rpi_cam = None  # For Picamera2

    if input_path == "camera":
        # Try RPi camera first (Picamera2)
        try:
            import picamera2
        ...
        logger.info(f"Using video file: {input_path}")
        ...
        try:
            ...
            logger.error(e)
            sys.exit(1)
        logger.info(f"Using image directory: {input_path}")

    return cap, images, rpi_cam  # rpi_cam is None for non-RPi cases
...
def preprocess_from_rpi_cam(rpi_cam,
                        batch_size: int,
...
def preprocess_from_cap(cap: cv2.VideoCapture,
    ...
    while True:
        ...
        frame_idx += 1
        print('fff', frame_idx, frame.shape)
